...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--hit-before-s", type=float, default=5.0, help="Seconds before hit to include in window for hit regions"
    )
    p.add_argument(
        "--hit-after-s", type=float, default=15.0, help="Seconds after hit to include in window for hit regions"
    )
...
def _hit_regions(hit_times, t_end, before_s, after_s):
    candidate_regions = []
    for ht in hit_times:
        start = max(0.0, ht - before_s)
        end = min(t_end, ht + after_s)
        if end > start + 0.5:  # Ensure minimum region length
            candidate_regions.append((start, end))

    # Implement strict overlap discarding (Option A)
    ...
    # Create an array of intervals [start, end)
    intervals = np.array(candidate_regions)

    # Check for overlaps. If any two intervals overlap, discard both.
    # A more efficient way to do this is to check each interval against all others.
    # For simplicity and correctness with "strict discard", iterate and check.
    for i in range(len(intervals)):
        ...
        for j in range(len(intervals)):
            ...
            # Check for overlap: [start1, end1) and [start2, end2) overlap if
            # start1 < end2 AND start2 < end1
            if current_start < other_end and other_start < current_end:
                is_overlapping = True
                break
        if not is_overlapping:
            final_regions.append(tuple(intervals[i]))
...
class HitEditor:
    def __init__(self, args, processed_t, aggregate_signal, t_spec, f_spec,
                 ...
                 pt, sig_matrix, fs,
                  delay, exclude_before, hit_before_s, hit_after_s, mode):
        self.args = args
        ...
        self.hit_before_s = hit_before_s
        self.hit_after_s = hit_after_s
        self.mode = mode
        self.accepted = False
        self._hit_lines_ts = []
        self._hit_lines_bb = []
...
def run_fft_analysis(pt, sig_matrix, fs, hit_times, delay, exclude_before, hit_before_s, hit_after_s, mode, dataset, component, args):
    ...
    hit_regs = _hit_regions(np.array(hit_times_sorted), t_end, hit_before_s, hit_after_s)
...
def main() -> int:
    ...
    print(f"Loading {dataset_name} in purecomoving mode...")
    ...
    editor = HitEditor(
        ...
        hit_before_s=args.hit_before_s,
        hit_after_s=args.hit_after_s,
        mode=args.mode,
    )
