...
def _compute_baseline_curve(
    ...
    *,
    ...
    if mode == "none":
        return np.ones_like(reference_amp, dtype=float)
    if mode == "zero":
        return np.ones_like(reference_amp, dtype=float)
    if mode == "global-baseline":
        floor = _compute_global_baseline(reference_amp)
        return np.full_like(reference_amp, floor, dtype=float)
    if mode == "local-baseline":
        return _compute_local_baseline(reference_amp, baseline_window_bins)
...
def _apply_display_floor(
    ...
    *,
    ...
    baseline = _compute_baseline_curve(
        reference_amp,
        mode=mode,
...
def _flatten_display_baseline(
    ...
    *,
    ...
    baseline = np.where(np.isfinite(baseline) & (baseline > 0.0), baseline, 1.0)
    return amp / baseline
...
def _display_state_amp(
    *,
    ...
    amp = np.asarray(amp, dtype=float)
    reference_amp = raw_amp_by_component[component]
    display_amp = amp
    if len(removed_components) > 0:
        display_amp = _apply_display_floor(
            display_amp,
            reference_amp,
            mode=args.display_floor,
            baseline_window_bins=args.baseline_window_bins,
        )
    if args.flatten_baseline:
        display_amp = _flatten_display_baseline(
            display_amp,
            reference_amp,
    ...
    for row_idx, payload in enumerate(step_payloads):
        ...
        for col_idx, (panel_kind, panel_label, panel_amp) in enumerate(panels):
            ...
            _plot_frequency_image(
                ...
                linear_color_label=linear_color_label,
                log_color_label=log_color_label,
                show_colorbar=False,
                annotate_range=True,
            )
...
def _prompt_select_panel(payload: dict[str, object]) -> tuple[str, str, np.ndarray]:
    panel_map = {
        "source": (str(payload["source_label"]), np.asarray(payload["source_amp_raw"], dtype=float)),
        "before": (str(payload["before_label"]), np.asarray(payload["before_amp_raw"], dtype=float)),
        "after": (str(payload["after_label"]), np.asarray(payload["after_amp_raw"], dtype=float)),
    }
        ...
        step_payloads.append(
            {
                ...
                "source_amp_raw": remove_amp.copy(),
                ...
                "before_amp_raw": target_amp.copy(),
                ...
                "after_amp_raw": cleaned_target_amp.copy(),
            }
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--flatten-baseline",
        action="store_true",
        help="Flatten plotted/exported spectra by dividing by a smooth baseline estimate.",
    )
    parser.add_argument(
        "--flatten-mode",
        choices=["local-baseline", "global-baseline"],
        default="local-baseline",
        help="Baseline estimator used when --flatten-baseline is enabled. Default: local-baseline",
    )
    parser.add_argument(
        "--show-x",
        nargs="?",
...
def main() -> int:
    ...
    try:
        ...
        amplitude_label = "Baseline-Normalized Amplitude" if args.flatten_baseline else "Normalized Amplitude"
        log_color_label = "Baseline-Normalized Amplitude (dB)" if args.flatten_baseline else "Amplitude (dB)"
        ...
        print(f"Baseline flattening: {'enabled' if args.flatten_baseline else 'disabled'}")
        if args.flatten_baseline:
        ...
        if args.spectrasave is not None:
            ...
            saved = save_spectrum_msgpack(
                ...
                metadata={
                    ...
                    "displayFloor": args.display_floor,
                    "flattenBaseline": bool(args.flatten_baseline),
                    "flattenMode": args.flatten_mode,
                    "baselineWindowBins": int(args.baseline_window_bins),
        ...
        if args.one_fig:
            if args.full_image:
                fig = _plot_clean_step_images(
                    ...
                    title=title,
                    linear_color_label=amplitude_label,
                    log_color_label=log_color_label,
                )
            else:
