...
Usage:
    ...
    python3 analysis/GoHit/HitReview.py DATASET --component x --delay 1.0 --exclude-before 1.0
    python3 analysis/GoHit/HitReview.py DATASET --mode psd --hit-window 5.0
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--posthit",
        action="store_true",
        help="Use post-hit windows as the primary GoHit region mode. The review still shows both overlays.",
    )
...
def _inter_hit_regions(hit_times, t_end, delay, exclude_before):
    return [
        (region.start_s, region.stop_s)
        for region in build_interhit_regions(
            hit_times,
            t_stop_s=float(t_end),
            exclude_after_s=float(delay),
            exclude_before_s=float(exclude_before),
        )
    ]
...
def _hit_regions(hit_times, t_end, hit_window):
    return [
        (region.start_s, region.stop_s)
        for region in build_posthit_regions(
            hit_times,
            t_stop_s=float(t_end),
            window_s=float(hit_window),
        )
    ]
...
def main() -> int:
    ...
    if not args.no_load_hits:
        ...
        if catalog is not None:
            loaded = list(catalog.hit_times_s)
            if loaded:
                print(f"Loaded {len(loaded)} hits from GoHit catalog: {default_catalog_path(args.dataset)}")
                hit_times.extend(loaded)
        else:
            loaded = load_legacy_hit_times(args.dataset, args.component)
            if loaded:
                print(f"Loaded {len(loaded)} hits from legacy NL CSV for bootstrap")
                hit_times.extend(loaded)
    ...
    catalog = HitCatalog(
        ...
        hit_times_s=tuple(float(value) for value in final_hits),
        detector={
            "sliding_len_s": float(args.sliding_len_s),
            "broadband_min_hz": float(args.broadband_min_hz),
            "broadband_max_hz": float(args.broadband_max_hz),
            ...
            "min_hit_separation_s": float(args.min_hit_separation_s),
            "strength_window_s": float(args.strength_window_s),
            "region_mode": "posthit" if args.posthit else "interhit",
