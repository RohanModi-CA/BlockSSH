...
def main():
    ...
    if not json_path.exists():
        raise FileNotFoundError(f"Taxonomy JSON not found at {json_path}")

    with open(json_path, 'r') as f:
        taxonomy = json.load(f)

    fundamentals = taxonomy.get("fundamentals", [])
    gen1_children = taxonomy.get("children", [])
    gen2_children = taxonomy.get("gen2_children", [])

    print(f"Loaded {len(fundamentals)} Fundamentals, {len(gen1_children)} 1st-Gen, {len(gen2_children)} 2nd-Gen.")
    ...
    for f in fundamentals:
        ...
        ax.text(f, amps[idx] * 1.3, f"F({f:.2f})", color="tab:red", fontsize=8, rotation=90, va="bottom", ha="center")

    # Plot 1st Gen children
    for c in gen1_children:
        f = c["child_hz"]
        if f > 28.0: continue
    ...
    # Plot 2nd Gen children
    for c in gen2_children:
        f = c["child_hz"]
        if f > 28.0: continue
        ax.axvline(f, color="tab:green", linestyle=":", linewidth=1.5, alpha=0.8)
        idx = np.argmin(np.abs(freqs - f))
        label = f"C2({f:.2f})"
        ax.text(f, amps[idx] * 1.1, label, color="tab:green", fontsize=8, rotation=90, va="bottom", ha="center")
    ...
    custom_lines = [
        ...
        Line2D([0], [0], color="tab:green", lw=1.5, linestyle=":", label="2nd-Gen Children")
    ]
    ...
    ax.set_title(f"Gen-2 Taxonomy (N={len(fundamentals)} Fundamentals) - {args.dataset} ({args.component}, {args.bond_spacing_mode})")
    ax.set_xlabel("Frequency (Hz)")
    ax.set_ylabel("Flattened Amplitude")
    ax.grid(True, alpha=0.3)

    out_path = SCRIPT_DIR / f"{args.dataset}_taxonomy_gen2_plot.png"
