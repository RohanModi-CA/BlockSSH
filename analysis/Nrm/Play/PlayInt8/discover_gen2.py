...
def get_amp(f_target, peak_freqs, peak_amps):
    idx = np.argmin(np.abs(peak_freqs - f_target))
    return peak_amps[idx]
...
def main():
    ...
    # Use the pristine N=20 taxonomy (before demotion broke it)
    ...
    classified_hz = set(fundamentals) | set(c["child_hz"] for c in gen1_children)
    unclassified_peaks = [p for p in peak_freqs if not any(abs(p - c) < 1e-4 for c in classified_hz)]
    
    print(f"Found {len(unclassified_peaks)} unclassified peaks to scan for 2nd-Gen Cascades.")

    rng = np.random.default_rng(42)
    gen2_children = tax.get("gen2_children", [])
    if gen2_children is None: gen2_children = []
    
    print("\n--- Starting 2nd-Generation Discovery Sweep ---")
    
    for unclass_f in unclassified_peaks:
        amp_target = get_amp(unclass_f, peak_freqs, peak_amps)
        found = False
        
        for fi in fundamentals:
            amp_fi = get_amp(fi, peak_freqs, peak_amps)
            
            for child in gen1_children:
                cj = child["child_hz"]
                amp_cj = get_amp(cj, peak_freqs, peak_amps)
                
                # AMPLITUDE RULE: Target must be strictly smaller than BOTH parents
                if amp_target > amp_fi or amp_target > amp_cj:
                    continue
                ...
                for math_target, rel_type in targets:
                    if abs(math_target - unclass_f) <= tol_hz:
                        plv, p_val = test_triad(fi, cj, unclass_f, hit_times, y_agg, fs, processed_t, rng)
                        if p_val < args.p_thresh:
                            print(f"[+] DISCOVERED 2nd-Gen Child {unclass_f:.3f} Hz -> {rel_type} of F({fi:.3f}) and C1({cj:.3f}) (PLV={plv:.3f}, p={p_val:.3f})")
    ...
    print(f"\nSweep complete. Discovered {len(gen2_children)} 2nd-Generation Cascades.")
    
    tax["gen2_children"] = gen2_children
    out_path = SCRIPT_DIR / f"{args.dataset}_taxonomy_gen2.json"
