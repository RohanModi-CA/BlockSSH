...
try:
    from int1 import detect_observed_peaks
...
def plot_results(
    *,
    ...
    path = OUTPUT_DIR / "int3_phase_pipeline.png"
    fig.savefig(path)
    plt.close(fig)
    return path
...
def write_summary(
    *,
    ...
    path = OUTPUT_DIR / "int3_phase_pipeline_summary.txt"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path
...
def main() -> int:
    ...
    priors = [
        resolve_prior(0.42, observed_peaks, label="f0.42", role="fundamental", certainty="100%"),
        resolve_prior(8.96, observed_peaks, label="f8.96", role="fundamental", certainty="80%"),
        resolve_prior(12.0, observed_peaks, label="f12", role="fundamental", certainty="75%"),
        resolve_prior(16.63, observed_peaks, label="f16.63", role="fundamental", certainty="100%"),
        resolve_prior(23.10, observed_peaks, label="f23+", role="fundamental", certainty="100%"),
        resolve_prior(11.31, observed_peaks, label="u11.31", role="suspect_child", certainty="not fundamental?"),
        resolve_prior(13.99, observed_peaks, label="u14", role="child", certainty="100% not fundamental"),
        resolve_prior(18.35, observed_peaks, label="u18.35", role="child", certainty="100% not fundamental"),
    ]
    ...
    csv_path = write_csv(OUTPUT_DIR / "int3_phase_pipeline.csv", stage1_results + stage2_results)
