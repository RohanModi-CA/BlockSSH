...
def save_csv(observed_peaks: list[ObservedPeak], explanations: list[Explanation], summaries: list[dict[str, float]]) -> tuple[Path, Path, Path]:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    peaks_csv = OUTPUT_DIR / "observed_peaks.csv"
    ...
    expl_csv = OUTPUT_DIR / "explanations.csv"
    ...
    summ_csv = OUTPUT_DIR / "generator_ladder.csv"
...
def plot_spectrum_and_explanations(
    freqs: np.ndarray,
    amplitude: np.ndarray,
    observed_peaks: list[ObservedPeak],
    explanations: list[Explanation],
    ...
    ax.set_title("Averaged Welch Spectrum With Auto Peaks and First Explanation Stage")
    ...
    ax.grid(alpha=0.25)
    ax.legend()
    fig_path = OUTPUT_DIR / "int1_spectrum_ladder.png"
    ...
    ax.set_title(f"Top-{top_k} Generator Explanation Map")
    ax.set_xlabel("Frequency (Hz)")
    ax.set_ylabel("Amplitude")
    ax.grid(alpha=0.25)
    fig2_path = OUTPUT_DIR / "int1_topk_explanations.png"
...
def main() -> int:
    ...
    csv_paths = save_csv(observed_peaks, explanations, summaries)
    fig_paths = plot_spectrum_and_explanations(freqs, amplitude, observed_peaks, explanations, summaries)
    summary_path = write_summary(observed_peaks, explanations, summaries, fig_paths, csv_paths, args)

    ranked = sorted(observed_peaks, key=lambda peak: peak.amplitude, reverse=True)
    print(f"Detected {len(observed_peaks)} merged peaks in {args.min_freq_hz:.1f}-{args.max_freq_hz:.1f} Hz")
    print("Top generators:")
