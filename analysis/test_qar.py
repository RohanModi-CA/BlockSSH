...
def test_triad(f1, f2, f3):
    ...
    print(f'Triad {f1}+{f2}->{f3} | Max -log10(p) = {np.max(log_p):.2f}, Mean = {np.mean(log_p):.2f}, % windows > 3.0 (p<0.001) = {100*np.mean(log_p > 3.0):.1f}%')

test_triad(8.96, 9.42, 18.35)
test_triad(3.35, 8.96, 12.31)
test_triad(3.35, 8.96, 16.61)
test_triad(0.41, 12.0, 18.35)
